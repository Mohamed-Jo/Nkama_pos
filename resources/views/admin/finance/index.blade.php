            ...
            <details class="fin-details" open>
                ...
                <div class="fin-details-body">
                    <form method="POST" action="{{ route('admin.finance.expenses.store') }}" class="fin-form expense">
                        ...
                        <div class="fin-field"><label>Pagamento</label><select name="payment_method" required>@foreach($expensePaymentMethods as $method)<option value="{{ $method->code }}" @selected(old('payment_method', 'pending') === $method->code)>{{ $method->name }}</option>@endforeach</select></div>
